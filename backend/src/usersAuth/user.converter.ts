...
export async function mapDTOtoDomain(userDTO: UserDTO): Promise<User> {
    ...
    return new User(userDTO.firstName, userDTO.lastName, userDTO.email, await hashedPassword);
}

export async function mapDomainToDTO(user: User): Promise<UserDTO> {
    return new UserDTO(user.firstName, user.lastName, user.email, user.password);
}